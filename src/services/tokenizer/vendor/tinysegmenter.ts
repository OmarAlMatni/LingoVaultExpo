 ...
 * tinysegmenter.ts
 *
 * Vendored, mechanically transformed (function+prototype -> ES class) port of:
 *   TinySegmenter 0.2 -- Super compact Japanese tokenizer in Javascript
 *   (c) 2008 Taku Kudo <taku@chasen.org>
 *   Freely distributable under a new BSD licence:
 *   http://chasen.org/~taku/software/TinySegmenter/LICENCE.txt
 *
 * Source pulled from the published 'tiny-segmenter' npm package (MIT re-license
 * by the JS porter, same underlying BSD-licensed weight tables/algorithm).
 * https://www.npmjs.com/package/tiny-segmenter
 *
 * No dictionary files, no WASM, no native module, no network request --
 * it is a single pure-JS statistical segmenter (~18KB of hardcoded feature
 * weights) that ships as ordinary JS in the RN bundle. This is precisely
 * why it was chosen over Suzume (WASM -- unsupported in Hermes/RN without
 * a custom native bridge) and Kuromoji (multi-megabyte gzipped dictionary
 * files that caused the old Capacitor asset-loading problems).
 *
 * Do not hand-edit the weight tables below -- they are trained coefficients,
 * not application logic. All app-specific behavior (POS guessing, base-form
 * normalization) lives one layer up in TinySegmenterTokenizer.ts.
 */
